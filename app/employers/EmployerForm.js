...
export default function EmployerForm() {
  ...
  if (status === 'success') return <div className="alert alert-success">✅ Enquiry received! Our team will contact you within 24 business hours.</div>;
  return (
    <form onSubmit={handleSubmit(onSubmit)}>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:'1rem' }}>
        ...
        <div className="form-group"><label>Phone *</label><input {...register('phone',{required:true})} placeholder="+254 700 000 000" />{errors.phone && <span className="form-error">Required</span>}</div>
      </div>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:'1rem' }}>
        ...
        <div className="form-group"><label>Job Category</label><select {...register('jobCategory')}><option value="professional">Professional</option><option value="labour">Labour / Blue-Collar</option><option value="both">Both</option></select></div>
      </div>
      ...
      <button type="submit" className="btn btn-primary" style={{ width:'100%' }} disabled={isSubmitting}>{isSubmitting ? 'Sending...' : 'Submit Hiring Enquiry'}</button>
    </form>
  );
}
